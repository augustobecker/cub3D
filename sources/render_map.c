...
/*   Updated: 2023/05/01 20:08:30 by acesar-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/27 10:10:25 by acesar-l          #+#    #+#             */
/*   Updated: 2023/05/01 21:36:14 by acesar-l         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void identify_rect(t_data *data, int x, int y)
{
	...
	(data->map[y][x] != 'N') && (data->map[y][x] != 'S') && (data->map[y][x] != 'W') && (data->map[y][x] != 'E'))
		return;
	if (data->map[y][x] == ' ')
		render_rect(&data->img, x * MINIMAP_SCALE , y * MINIMAP_SCALE, BLACK_PIXEL);
	if (data->map[y][x] == '1')
		render_rect(&data->img, x * MINIMAP_SCALE, y * MINIMAP_SCALE, WALL_COLOR);
	if (data->map[y][x] == '0')
		render_rect(&data->img, x * MINIMAP_SCALE, y * MINIMAP_SCALE, FLOOR_COLOR);
	if ((data->map[y][x] == 'N') 
	|| (data->map[y][x] == 'S') 
	|| (data->map[y][x] == 'W') 
	|| (data->map[y][x] == 'E'))
